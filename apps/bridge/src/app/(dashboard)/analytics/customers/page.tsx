  ...
  return (
    <main style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
            <div style={{ background: 'var(--d-surface)', border: '1px solid var(--d-border)', borderRadius: 10, padding: '20px 24px' }}>
              <ChartTitle title={`Bug reports by ${fieldConfig?.field2Label || 'field 2'}`} info="Number of bug report tickets per value in the second configurable dropdown. Highlights which options cause the most friction." />
              {data.frictionByField2.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={data.frictionByField2.slice(0, 8).map((r) => ({ value: labelForValue(r.value, fieldConfig?.field2Options ?? []), count: r.count }))} layout="vertical" margin={{ top: 0, right: 40, left: 0, bottom: 0 }}>
