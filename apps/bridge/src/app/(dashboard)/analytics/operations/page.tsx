...
import { useFieldConfig, labelForValue } from '@/lib/useFieldConfig'
...
export default function AnalyticsPage() {
  const { token } = useAuth()
  const fieldConfig = useFieldConfig(token)
  ...
  const field1Data = (data?.byField1 ?? []).slice(0, 8).map((c) => ({ name: labelForValue(c.value, fieldConfig?.field1Options ?? []), value: c.count, color: '#3B82F6' }))
  const field2Data = (data?.byField2 ?? []).slice(0, 8).map((c) => ({ name: labelForValue(c.value, fieldConfig?.field2Options ?? []), value: c.count, color: '#3B82F6' }))
  const field1Title = `Top by ${fieldConfig?.field1Label || 'field 1'}`
  const field2Title = `Top by ${fieldConfig?.field2Label || 'field 2'}`
    ...
    <main style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${breakdownCols}, 1fr)`, gap: 14, marginBottom: 14 }}>
            ...
            {loading ? (
              <Card title={field1Title}><Skeleton h={140} /></Card>
            ) : field1Data.length > 0 ? (
              <Card title={field1Title} subtitle="Breakdown by first configurable dropdown" info="Ticket distribution by the first configurable dropdown (field1), rendered only when data exists.">
                <ResponsiveContainer width="100%" height={140}>
            ...
            {loading ? (
              <Card title={field2Title}><Skeleton h={140} /></Card>
            ) : field2Data.length > 0 ? (
              <Card title={field2Title} subtitle="Breakdown by second configurable dropdown" info="Ticket distribution by the second configurable dropdown (field2), rendered only when data exists.">
                <ResponsiveContainer width="100%" height={140}>
