...
/** Derive a stable dropdown `value` key from a human label. Generated once for a new
 * option and then frozen — existing values are never re-derived, so renaming a label
 * never changes the key tickets/analytics are grouped by. */
...
export default function BrandingPage() {
  ...
  // Dropdown-option keys that are already persisted. Their `value` is frozen so a label
  // rename never re-keys existing tickets/analytics; only brand-new options derive a value.
  const [lockedValues, setLockedValues] = useState<{ 1: Set<string>; 2: Set<string> }>({ 1: new Set(), 2: new Set() })
  // Ticket counts per stored field value — powers the delete-option warning.
  const [usage, setUsage] = useState<{ field1: Record<string, number>; field2: Record<string, number> } | null>(null)
  // Pending option deletion awaiting confirmation (only armed for in-use, persisted options).
  const [pendingDelete, setPendingDelete] = useState<{ field: 1 | 2; idx: number } | null>(null)
  ...
  useEffect(() => {
    ...
    api.get<AppConfig>('/config', token)
      .then((res) => {
        ...
        setLockedValues({
          1: new Set((res.field1Options ?? []).map((o) => o.value)),
          2: new Set((res.field2Options ?? []).map((o) => o.value)),
        })
        if (res.logoUrl) setLogoPreview(res.logoUrl)
      }).catch(console.error)
    api.get<{ field1: Record<string, number>; field2: Record<string, number> }>('/config/field-usage', token)
      .then(setUsage)
      .catch(() => {})
  }, [token])
  ...
  const save = async () => {
    ...
    try {
      ...
      await api.patch('/config', {
        ...
        field1Options: form.field1Options.filter((o) => o.value && o.label),
        field2Label: form.field2Label || null,
        field2Options: form.field2Options.filter((o) => o.value && o.label),
      }, token)
      // Freeze the just-saved option keys so a later label edit won't re-derive them.
      setLockedValues({
        1: new Set(form.field1Options.map((o) => o.value)),
        2: new Set(form.field2Options.map((o) => o.value)),
      })
      setIsSaved(true)
  ...
  return (
    <div style={{ maxWidth: 920 }}>
            ...
            {([1, 2] as const).map((n) => {
              ...
              return (
                <div key={n} style={{ marginBottom: n === 1 ? 20 : 0 }}>
                  ...
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--d-text-3)', marginBottom: 4 }}>
                    Options
                  </label>
                  <p style={{ fontSize: 11, color: 'var(--d-text-4)', margin: '0 0 8px', lineHeight: 1.5 }}>
                    Renaming an option only changes how it&apos;s displayed — past tickets and analytics stay linked to its key.
                  </p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {options.map((opt, idx) => {
                      const locked = lockedValues[n].has(opt.value)
                      const count = (n === 1 ? usage?.field1 : usage?.field2)?.[opt.value] ?? 0
                      return (
                      <div key={idx} style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              const next = [...options]
                              // New options derive their key from the label; persisted keys are frozen.
                              next[idx] = locked ? { ...next[idx], label } : { ...next[idx], label, value: slugify(label) }
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              // Only persisted, in-use options need a confirmation; drop the rest immediately.
                              if (locked && count > 0) { setPendingDelete({ field: n, idx }); return }
                              setForm(f => ({ ...f, [optionsKey]: f[optionsKey].filter((_, i) => i !== idx) }))
                            }}
                        ...
                        <span style={{ fontSize: 10.5, color: 'var(--d-text-4)', fontFamily: 'var(--font-mono)', paddingLeft: 2 }}>
                          key: {opt.value || '—'}{locked && count > 0 ? ` · ${count} ticket${count === 1 ? '' : 's'}` : ''}
                        </span>
                      </div>
                      )
                    })}
      ...
      {pendingDelete && (() => {
        const { field, idx } = pendingDelete
        const optionsKey = `field${field}Options` as 'field1Options' | 'field2Options'
        const opt = form[optionsKey][idx]
        const count = (field === 1 ? usage?.field1 : usage?.field2)?.[opt?.value ?? ''] ?? 0
        const confirmDelete = () => {
          setForm(f => ({ ...f, [optionsKey]: f[optionsKey].filter((_, i) => i !== idx) }))
        ...
        return (
          <div
            ...
            style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          >
            <div onClick={(e) => e.stopPropagation()} style={{ ...cardStyle, maxWidth: 420, width: '100%' }}>
              <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--d-text)', margin: '0 0 8px' }}>
                Remove “{opt?.label}”?
              </h3>
              <p style={{ fontSize: 13, color: 'var(--d-text-3)', margin: '0 0 18px', lineHeight: 1.6 }}>
                {count} ticket{count === 1 ? '' : 's'} {count === 1 ? 'uses' : 'use'} this option. Removing it won&apos;t delete
                any ticket data or change analytics — but {count === 1 ? 'that ticket' : 'those tickets'} will display the raw
                key <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--d-text-2)' }}>{opt?.value}</span> instead of a label.
              </p>
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                <button
                  type="button"
                  onClick={() => setPendingDelete(null)}
                  style={{ height: 34, padding: '0 14px', background: 'var(--d-surface)', border: '1px solid var(--d-border)', borderRadius: 'var(--r-sm)', fontSize: 13, color: 'var(--d-text-2)', cursor: 'pointer', fontFamily: 'inherit' }}
                >
                  Cancel
                </button>
                <button
                  ...
                  style={{ height: 34, padding: '0 14px', background: 'var(--d-danger)', border: '1px solid var(--d-danger)', borderRadius: 'var(--r-sm)', fontSize: 13, fontWeight: 600, color: '#fff', cursor: 'pointer', fontFamily: 'inherit' }}
                >
                  Remove option
                </button>
              </div>
