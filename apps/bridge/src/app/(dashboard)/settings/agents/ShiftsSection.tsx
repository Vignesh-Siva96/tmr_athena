...
// ─── Shifts section (rendered inside Agent settings) ────────────────────────────
...
export function ShiftsSection() {
  ...
  const [timezoneSaveState, setTimezoneSaveState] = useState<'idle' | 'saving' | 'saved'>('idle')
  const timezoneDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  ...
  const saveTimezone = async (tz: string) => {
    ...
    setTimeout(() => setTimezoneSaveState('idle'), 2000)
  }

  const handleTimezoneChange = (tz: string) => {
    setTimezone(tz)
    if (timezoneDebounceRef.current) clearTimeout(timezoneDebounceRef.current)
    timezoneDebounceRef.current = setTimeout(() => { void saveTimezone(tz) }, 500)
  }
  ...
  return (
    <div style={{ marginTop: 32, padding: 20, background: 'var(--d-surface)', border: '1px solid var(--d-border)', borderRadius: 'var(--r-lg)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(59,130,246,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <Calendar size={18} style={{ color: 'var(--d-accent)' }} />
          </div>
          <div>
            <h2 style={{ margin: 0, fontSize: 15, fontWeight: 600, color: 'var(--d-text)' }}>Shifts</h2>
            <p style={{ margin: '2px 0 0', fontSize: 13, color: 'var(--d-text-3)' }}>On-call windows for primary-agent routing of escalated tickets.</p>
          </div>
        </div>
        <button
          style={{ ...btnStyle, background: 'var(--d-accent)', color: '#fff', border: 'none', display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}
      ...
      {/* Timezone selector */}
      <div style={{ background: 'var(--d-raised)', border: '1px solid var(--d-border)', borderRadius: 10, padding: 16, marginBottom: 16 }}>
        <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: 'var(--d-text)', marginBottom: 8 }}>Timezone</label>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {timezoneSaveState === 'saving' && (
            <span style={{ fontSize: 12, color: 'var(--d-text-4)' }}>Saving…</span>
          )}
          {timezoneSaveState === 'saved' && (
            <span style={{ fontSize: 12, color: 'var(--d-success, #16a34a)' }}>Saved ✓</span>
          )}
          {timezoneSaveState === 'idle' && (
      ...
      {/* New shift form */}
      {showForm && (
        <div style={{ background: 'var(--d-raised)', border: '1px solid var(--d-accent)', borderRadius: 10, padding: 16, marginBottom: 16 }}>
      ...
      {/* Shifts list */}
      {loading ? (
        <div style={{ background: 'var(--d-raised)', border: '1px solid var(--d-border)', borderRadius: 10, overflow: 'hidden' }}>
        ...
        <div style={{ background: 'var(--d-raised)', border: '1px solid var(--d-border)', borderRadius: 10, padding: 40, textAlign: 'center' }}>
