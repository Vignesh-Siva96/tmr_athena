...
type AgentRole = 'ADMIN' | 'PRIMARY_AGENT' | 'SECONDARY_AGENT'

const ROLE_OPTIONS: { value: AgentRole; label: string }[] = [
  { value: 'ADMIN', label: 'Admin' },
  { value: 'PRIMARY_AGENT', label: 'Primary agent' },
  { value: 'SECONDARY_AGENT', label: 'Secondary agent' },
]
...
interface AgentRow { id: string; email: string; name: string; role: AgentRole; isActive: boolean; lastActiveAt: string | null; inviteAccepted: boolean }
...
export default function AgentsSettingsPage() {
  ...
  const [inviteRole, setInviteRole] = useState<AgentRole>('PRIMARY_AGENT')
  ...
  const colTemplate = isAdmin ? '40px 1fr 200px 160px 100px 160px' : '40px 1fr 200px 140px 100px'
          ...
          agents.map((a) => (
            <div key={a.id} style={{ display: 'grid', gridTemplateColumns: colTemplate, gap: 0, padding: '12px 16px', borderBottom: '1px solid var(--d-border-2)', alignItems: 'center' }}>
              ...
              {isAdmin ? (
                <select
                  value={a.role}
                  onChange={(e) => { void handleRoleChange(a.id, e.target.value as AgentRole) }}
                  style={{ height: 28, padding: '0 6px', background: 'var(--d-surface)', border: '1px solid var(--d-border)', borderRadius: 'var(--r-sm)', fontSize: 12, color: 'var(--d-text)', outline: 'none', fontFamily: 'inherit', cursor: 'pointer', width: 150 }}
                >
                  {ROLE_OPTIONS.map((o) => (
                    <option key={o.value} value={o.value}>{o.label}</option>
                  ))}
