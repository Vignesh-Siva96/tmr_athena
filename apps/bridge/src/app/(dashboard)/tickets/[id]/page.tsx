...
import { TmrProductSection } from '@/components/dashboard/TmrProductSection'
import type { TmrMetadata } from '@/components/dashboard/TmrProductSection'
...
const STATUS_CLS: Record<TicketStatus, string> = { NEW: 'd-new', OPEN: 'd-open', IN_PROGRESS: 'd-prog', WAITING: 'd-wait', RESOLVED: 'd-res', CLOSED: 'd-closed', DISMISSED: 'd-closed' }
...
interface UserTmrProfile {
  tmrMetadata: TmrMetadata | null
  tmrMetadataStatus: 'PENDING' | 'OK' | 'NOT_FOUND' | 'ERROR'
  tmrMetadataAt: string | null
}
...
export default function AgentTicketPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [userTmr, setUserTmr] = useState<UserTmrProfile | null>(null)
  ...
  useEffect(() => {
    if (!token) return
    api.get<{ oauthEmail?: string | null; supportEmail?: string | null } & Partial<FieldConfig>>('/config', token)
      .then((cfg) => {
        setSupportEmail(cfg.supportEmail ?? cfg.oauthEmail ?? '')
        setFieldConfig({
  ...
  // Load TMR profile for the ticket's customer once we know who they are
  const loadUserTmr = useCallback((userId: string) => {
    if (!token) return
    return api.get<{ user: UserTmrProfile }>(`/users/${userId}`, token)
      .then((res) => setUserTmr({
        tmrMetadata: res.user.tmrMetadata,
        tmrMetadataStatus: res.user.tmrMetadataStatus,
        tmrMetadataAt: res.user.tmrMetadataAt,
      }))
      .catch(() => {})
  }, [token])

  useEffect(() => {
    if (!ticket?.user?.id || !token) return
    loadUserTmr(ticket.user.id)
  }, [ticket?.user?.id, token, loadUserTmr])

  // Lazy sync PENDING TMR status on first open
  useEffect(() => {
    if (!ticket?.user?.id || !token || tmrLazySyncedRef.current) return
    if (userTmr?.tmrMetadataStatus === 'PENDING') {
      tmrLazySyncedRef.current = true
      setTmrSyncing(true)
      api.post(`/users/${ticket.user.id}/tmr-metadata/refresh`, {}, token)
        .then(() => { setTimeout(() => { void loadUserTmr(ticket.user.id)?.finally(() => setTmrSyncing(false)) }, 3000) })
        .catch(() => setTmrSyncing(false))
    }
  }, [userTmr?.tmrMetadataStatus, ticket?.user?.id, token, loadUserTmr])

  const handleTmrRefresh = useCallback(() => {
    if (!ticket?.user?.id || !token) return
    setTmrSyncing(true)
    api.post(`/users/${ticket.user.id}/tmr-metadata/refresh`, {}, token)
      .then(() => { setTimeout(() => { void loadUserTmr(ticket.user.id)?.finally(() => setTmrSyncing(false)) }, 2500) })
      .catch(() => setTmrSyncing(false))
  }, [ticket?.user?.id, token, loadUserTmr])
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      // Include CC on replies (not notes)
      if (composeTab !== 'note') payload.cc = ccList
      ...
      // Sync ccList from the message the server persisted (respects server-side drops)
      if (composeTab !== 'note' && res.message.cc !== undefined) {
        setCcList(res.message.cc)
      }
      setBody('')
      setReplyAttachments([])
  ...
  return (
    <>
          ...
          {showCompose && (
            <div ref={composeRef} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
              ...
              <div style={{ flex: 1, background: composeTab === 'note' ? 'var(--d-note-bg)' : 'var(--d-surface)', border: composeTab === 'note' ? '1px solid rgba(245,158,11,0.3)' : '1px solid var(--d-border)', borderRadius: 8, boxShadow: '0 2px 8px rgba(0,0,0,0.15)', overflow: 'hidden' }}>
                ...
                {/* CC chip editor — reply only */}
                {composeTab === 'reply' && (
                  <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 4, padding: '6px 14px', borderBottom: '1px solid var(--d-border-2)', minHeight: 34 }}>
                    <span style={{ fontSize: 11, color: 'var(--d-text-4)', flexShrink: 0, marginRight: 2 }}>Cc:</span>
                    {ccList.map((email) => (
                      <div key={email} style={{ display: 'inline-flex', alignItems: 'center', gap: 3, padding: '2px 8px', background: 'var(--d-raised)', border: '1px solid var(--d-border)', borderRadius: 4, fontSize: 11, color: 'var(--d-text-2)' }}>
                        <span>{email}</span>
                        <button type="button" onClick={() => setCcList((prev) => prev.filter((e) => e !== email))}
                    ...
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ',' || e.key === 'Tab') {
                          e.preventDefault()
                          const val = ccInput.trim().toLowerCase().replace(/,$/, '')
                          if (val && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val) && !ccList.includes(val)) {
                      ...
                      style={{ flex: 1, minWidth: 100, border: 'none', outline: 'none', background: 'transparent', fontSize: 11, color: 'var(--d-text)', fontFamily: 'inherit' }}
                    />
                  </div>
                )}
                {/* Rich text editor */}
      ...
      <aside style={{ width: 280, flexShrink: 0, overflowY: 'auto', borderLeft: '1px solid var(--d-border)', background: 'var(--d-bg)', padding: 16, display: 'flex', flexDirection: 'column', gap: 0 }}>
        ...
        {/* TMR Product Account — compact widget */}
        {userTmr && userTmr.tmrMetadataStatus !== 'NOT_FOUND' && (
          <TmrProductSection
            variant="compact"
            data={userTmr.tmrMetadata}
            status={userTmr.tmrMetadataStatus}
            syncedAt={userTmr.tmrMetadataAt}
            onRefresh={handleTmrRefresh}
            onViewMore={() => setShowProfile(true)}
            syncing={tmrSyncing}
          />
        )}
        ...
        <div style={{ paddingBottom: 16, borderBottom: '1px solid var(--d-border-2)', marginBottom: 16 }}>
          ...
          {[
            ...
            ...(ticket.field1 ? [[fieldConfig?.field1Label || 'Field 1', labelForValue(ticket.field1, fieldConfig?.field1Options ?? [])]] : []),
            ...(ticket.field2 ? [[fieldConfig?.field2Label || 'Field 2', labelForValue(ticket.field2, fieldConfig?.field2Options ?? [])]] : []),
        ...
        {/* Tags */}
        <div style={{ paddingBottom: 16, borderBottom: '1px solid var(--d-border-2)', marginBottom: 16 }}>
          <p style={{ fontSize: 10, fontWeight: 600, color: 'var(--d-text-4)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>Tags</p>
