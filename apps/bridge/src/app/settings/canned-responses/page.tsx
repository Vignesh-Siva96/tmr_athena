...
function RichEditor({
  ...
  // Only run when initialHtml changes externally (e.g. switching edit targets)
  ...
  return (
    <div style={{ padding: 16, background: 'var(--d-raised)', border: '1px solid var(--d-border)', borderRadius: 'var(--r-lg)', marginBottom: 16 }}>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <RichEditor initialHtml={body} onHtmlChange={setBody} placeholder="Write the template body…" />
      </div>
      <div style={{ display: 'flex', gap: 8 }}>
        <button
          type="button"
          onClick={() => { if (name.trim() && body.replace(/<[^>]*>/g, '').trim()) onSave(name.trim(), body) }}
          disabled={saving || !name.trim() || !body.replace(/<[^>]*>/g, '').trim()}
          style={{ height: 34, padding: '0 16px', background: 'var(--d-accent)', color: '#fff', border: 'none', borderRadius: 'var(--r-sm)', fontSize: 13, fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1, fontFamily: 'inherit', whiteSpace: 'nowrap' }}
        >
          {saving ? 'Saving…' : 'Save'}
        </button>
        <button
  ...
  return (
    <div style={{ maxWidth: 760 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: showCreate ? 16 : 28 }}>
        ...
        >
          <Plus size={14} /> {showCreate ? 'Cancel' : 'New template'}
        </button>
      </div>
