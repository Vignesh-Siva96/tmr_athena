...
export default function TagsSettingsPage() {
  ...
  const handleDelete = async (tag: Tag) => {
    if (!token) return
    const msg = tag._count.tickets > 0
      ? `Delete tag "${tag.name}"? It is assigned to ${tag._count.tickets} ticket${tag._count.tickets !== 1 ? 's' : ''}. Deleting will remove it from those tickets.`
      : `Delete tag "${tag.name}"?`
  ...
  return (
    <div style={{ maxWidth: 700 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: showCreate ? 16 : 28 }}>
        ...
        >
          <Plus size={14} /> {showCreate ? 'Cancel' : 'New tag'}
        </button>
      </div>
