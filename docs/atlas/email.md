...
- **`sendAgentReply`** sends the reply body and the sign-off — **no quoted block**. Returns `{ messageId, quotedMessageIds: [] }`. The body is sent as **`html`** (`message.bodyHtml ?? message.body` — bot replies carry pre-rendered `bodyHtml`; agent replies store HTML directly from the rich-text editor) with a tag-stripped **`text`** fallback via `htmlToPlainText()` (keeps link URLs as `label (url)`, list items as `- ` bullets). This is what prevents outbound replies arriving as literal `<ul><li>` tags. See R258. `sendPortalReplyAck` gets the same `html`+`text` treatment so a customer's formatted portal reply isn't echoed back as tags. `sendAgentReply` also attaches the message's **file attachments**: `loadEmailAttachments(messageId)` queries non-link rows (`attachment.findMany {messageId, isLink:false}`), fetches each blob from MinIO via `FilesService.getAttachmentBuffer` (object key recovered from the presigned URL path), and adds them as nodemailer attachment parts — so agent-uploaded files reach the customer, not just the portal. A file that fails to fetch is logged and dropped without failing the send. See R260.
